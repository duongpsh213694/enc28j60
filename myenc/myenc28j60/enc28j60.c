...
static void enc28j60_set_multicast_list(struct net_device *dev)
{
	...
	if (dev->flags & IFF_PROMISC) {
		if (netif_msg_link(priv))
			netdev_info(dev, "che do lon xon\n");
		priv->rxfilter = RXFILTER_PROMISC;
	} else if ((dev->flags & IFF_ALLMULTI) || !netdev_mc_empty(dev)) {
		if (netif_msg_link(priv))
			netdev_info(dev, "%sche do multicast\n",
				    (dev->flags & IFF_ALLMULTI) ? "all-" : "");
		priv->rxfilter = RXFILTER_MULTI;
	} else {
		if (netif_msg_link(priv))
			netdev_info(dev, "che do binh thuong\n");
		priv->rxfilter = RXFILTER_NORMAL;
	}
...
static void enc28j60_setrx_work_handler(struct work_struct *work)
{
	...
	if (priv->rxfilter == RXFILTER_PROMISC) {
		if (netif_msg_drv(priv))
			dev_printk(KERN_DEBUG, dev, "che do lon xon\n");
		locked_regb_write(priv, ERXFCON, 0x00);
	} else if (priv->rxfilter == RXFILTER_MULTI) {
		if (netif_msg_drv(priv))
			dev_printk(KERN_DEBUG, dev, "che do multicast\n");
		locked_regb_write(priv, ERXFCON,
					ERXFCON_UCEN | ERXFCON_CRCEN |
					ERXFCON_BCEN | ERXFCON_MCEN);
	} else {
		if (netif_msg_drv(priv))
			dev_printk(KERN_DEBUG, dev, "che do binh thuong\n");
		locked_regb_write(priv, ERXFCON,
					ERXFCON_UCEN | ERXFCON_CRCEN |
					ERXFCON_BCEN);
	}
...
static int enc28j60_probe(struct spi_device *spi)
{
	...
	if (netif_msg_drv(&debug))
		dev_info(&spi->dev, "Driver Ethernet Phien Ban %s da duoc nap\n", DRV_VERSION);
...
static void enc28j60_remove(struct spi_device *spi)
{
	...
	dev_info(&spi->dev, "Da Go Driver, Cam On Ban Da Dung");
